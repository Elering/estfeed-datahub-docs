...
| customerType           | Customer Type                       |                                           | yes                     | One of: Electricuty: CONSUMER, GRID_OPERATOR, PRODUCER, MICRO (micro producer), LINE_OPERATOR, ENERGY_STORAGE_UNIT, CHARGING_POINT_OPERATOR. Gas: CONSUMER - not protected consumer (slow disconnection time >6h), CONSUMER_2 - not protected consumer (fast disconnection time < 6h), GRID_OPERATOR, PRODUCER, PROTECTED - protected consumer (residential customer), PROTECTED_OTHER - protected consumer (other), GENERAL_INTEREST_SERVICE_PROVIDER, POWER_PRODUCER_FROM_GAS.                                                                                                |
...
|-----------------------|-------------------------------------|--------------------------------------------|--------------------------|--------------------------|--------------------------------------------------------------------|
| productionSource| Production Source | | yes, if production=true | yes, if production=true | One of: SOLAR, WIND, HYDRO, BIOGAS, BIOMASS, NATURAL_GAS, OIL_SHALE, OTHER_RENEWABLE, OTHER_NON_RENEWABLE.                |
| transmissionNetworkEic | Transmission Network EIC | 16 digit EIC code of transmission network | yes | yes | EIC code of transmission network must be registered in the Datahub. Commodity type has to match Metering Point's commodity type. In case the correct code is missing it is possible to use the general code 38A-1234567890-1.  |
| isolatedMeteringPoint | Isolated Metering Point             | is isolated metering point?                | yes                      | yes                      | One of: TRUE (yes), FALSE (no)                                     |
| electricalHeating     | Electrical Heating                  | electrical heating used in metering point? | yes                      | yes                      | One of: TRUE (yes), FALSE (no)                                     |
| chargingPoint         | Charging Point                      | is charging point?                         | yes                      | yes                      | One of: TRUE (yes), FALSE (no)                                     |
| storageCapacity       | Storage Capacity                    | storage capacity in kW                     | no                       | yes                      | Must be integer or floating (max. 2 positions after comma) number. |
| storageEnergy         | Storage Energy                      | storage capacity  energy in kWh            | no                       | yes                      | Must be integer or floating (max. 2 positions after comma) number. |
| productionCapacity    | Production Capacity                 | production capacity in kW                  | no                       | yes                      | Must be integer or floating (max. 2 positions after comma) number. |
| transmissionCapacity  | Transmission Capacity               | transmission capacity in kW                | no                       | yes                      | Must be integer or floating (max. 2 positions after comma) number. |

- Specific data for aggregation metering points:

| Attribute in the API      | Column name in mass import template | Explanation                                | Mandatory in V1 version? | Mandatory in V2 version?         | Other rules                                                                                                                                           |
|---------------------------|-------------------------------------|--------------------------------------------|--------------------------|----------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------|
| parentMeterEic            | Parent Metering Point EIC           | 16 digit EIC code of parent metering point | yes                      | yes                              | Must be registered in Datahub, and the metering point owner's market role must be either GRID_OPERATOR or CLOSED_DISTRIBUTION_NETWORK. Cannot be updated                                                                                                      |
| aggregationProductType    | Agregation product type             | Agregation product type                    | cannot be used           | yes                              | One of: MFRR, AFRR, FCR, D1, D, LOCAL_PRODUCT. Can be changed only from "None" to actual value. Other updates are not allowed.                        |
| assetType                 | Asset type                          | Asset type                                 | cannot be used           | yes                              | One of: CONSUMPTION, PRODUCTION. Can be changed only from "None" to actual value. Other updates are not allowed.                                      |
| aggregationProductionType | Aggregation production type         | Metering point production type             | cannot be used           | yes, if asset type is PRODUCTION | One of: BESS, BV, WIND, HYDRO, OTHER. Can be changed only from "None" to actual value and if asset type is PRODUCTION. Other updates are not allowed. |
| resourceEic               | Resource EIC                        | 16 digit EIC code of the resource          | cannot be used           | yes                              | Doesn't have to be registered in Datahub. Can be changed only from "None" to actual value. Other updates are not allowed.                             |
...
| locality             | Locality                            | ADS level 3                                   | no                                       | no                              |                                                                                                                                                                        |
