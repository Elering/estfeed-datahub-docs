...
- **PAGE "OS_GO"**:
  ...
  - Net amounts entering the grid from end customers, kWh. For bidirectional metering points with net quantities (netIn and netOut) submitted to the Estfeed Datahub, the report uses these net values. For all other metering points, the standard in and out quantities are taken into account. In the report, both net quantities and standard quantities are aggregated in the netInQuantityPortfolio and netOutQuantityPortfolio columns. (netInQuantityPortfolio) (electricity market only)
  - Net amounts leaving the grid to end customers, kWh. For bidirectional metering points with net quantities (netIn and netOut) submitted to the Estfeed Datahub, the report uses these net values. For all other metering points, the standard in and out quantities are taken into account. In the report, both net quantities and standard quantities are aggregated in the netInQuantityPortfolio and netOutQuantityPortfolio columns. (netOutQuantityPortfolio) (electricity market only)
  ...
  - Net amounts entering the isolated grid from end customers, kWh. For bidirectional isolated grid metering points with net quantities (netIn and netOut) submitted to the Estfeed Datahub, the report uses these net values. For all other isolated metering points, the standard in and out quantities are taken into account. In the report, both net quantities and standard quantities are aggregated in the netInQuantityIsolated and netOutQuantityIsolated columns. (netInQuantityIsolated) (electricity market only)
  - Net amounts leaving the isolated grid to end customers, kWh For bidirectional isolated grid metering points with net quantities (netIn and netOut) submitted to the Estfeed Datahub, the report uses these net values. For all other isolated metering points, the standard in and out quantities are taken into account. In the report, both net quantities and standard quantities are aggregated in the netInQuantityIsolated and netOutQuantityIsolated columns. (netOutQuantityIsolated) (electricity market only)
...
- **Page "BH_GO"**
  ...
  - Amounts entering the grid outside the portfolio, m³. Volume of gas in cubic metres. (inQuantityM3) (gas market only)
  - Amounts leaving the grid outside the portfolio, m³. Volume of gas in cubic metres. (outQuantityM3) (gas market only)
  - Net amounts entering the grid outside the portfolio, kWh. For bidirectional grid metering points with net quantities (netIn and netOut) submitted to the Estfeed Datahub, the report uses these net values. For all other metering points, the standard in and out quantities are taken into account. In the report, both net quantities and standard quantities are aggregated in the netInQuantity and netOutQuantity columns. (netInQuantity) (electricity market only).
  - Net amounts leaving the grid outside the portfolio, kWh. For bidirectional grid metering points with net quantities (netIn and netOut) submitted to the Estfeed Datahub, the report uses these net values. For all other metering points, the standard in and out quantities are taken into account. In the report, both net quantities and standard quantities are aggregated in the netInQuantity and netOutQuantity columns. (netOutQuantity) (electricity market only).
  ...
  - Net amounts entering the isolated grid outside the portfolio, kWh. For bidirectional isolated grid metering points with net quantities (netIn and netOut) submitted to the Estfeed Datahub, the report uses these net values. For all other isolated metering points, the standard in and out quantities are taken into account. In the report, both net quantities and standard quantities are aggregated in the netInQuantityIsolated and netOutQuantityIsolated columns. (netInQuantityIsolated) (electricity market only).
  - Net amounts leaving the isolated grid outside the portfolio, kWh. For bidirectional isolated grid metering points with net quantities (netIn and netOut) submitted to the Estfeed Datahub, the report uses these net values. For all other isolated metering points, the standard in and out quantities are taken into account. In the report, both net quantities and standard quantities are aggregated in the netInQuantityIsolated and netOutQuantityIsolated columns. (netOutQuantityIsolated) (electricity market only).
  - Number of metering points, pcs (meteringPointsTotal)
- **Page "BH_OS"**
  ...
  - Amounts entering the grid, m³. Volume of gas in cubic metres. (inQuantityM3) (gas market only)
  - Amounts leaving the grid, m³. Volume of gas in cubic metres. (outQuantityM3) (gas market only)
  - Net amounts entering the grid, kWh. For bidirectional grid metering points with net quantities (netIn and netOut) submitted to the Estfeed Datahub, the report uses these net values. For all other metering points, the standard in and out quantities are taken into account. In the report, both net quantities and standard quantities are aggregated in the netInQuantity and netOutQuantity columns. (netInQuantity) (electricity market only).
  - Net amounts leaving the grid, kWh. For bidirectional grid metering points with net quantities (netIn and netOut) submitted to the Estfeed Datahub, the report uses these net values. For all other metering points, the standard in and out quantities are taken into account. In the report, both net quantities and standard quantities are aggregated in the netInQuantity and netOutQuantity columns. (netOutQuantity) (electricity market only).
  ...
  - Net amounts entering the isolated grid, kWh. For bidirectional isolated grid metering points with net quantities (netIn and netOut) submitted to the Estfeed Datahub, the report uses these net values. For all other isolated metering points, the standard in and out quantities are taken into account. In the report, both net quantities and standard quantities are aggregated in the netInQuantityIsolated and netOutQuantityIsolated columns. (netInQuantityIsolated) (electricity market only).
  - Net amounts leaving the isolated grid, kWh. For bidirectional isolated grid metering points with net quantities (netIn and netOut) submitted to the Estfeed Datahub, the report uses these net values. For all other isolated metering points, the standard in and out quantities are taken into account. In the report, both net quantities and standard quantities are aggregated in the netInQuantityIsolated and netOutQuantityIsolated columns. (netOutQuantityIsolated) (electricity market only).
...
- **PAGE "BH_OS_GO"**
  ...
  - Net amounts entering the grid, kWh. For bidirectional grid metering points with net quantities (netIn and netOut) submitted to the Estfeed Datahub, the report uses these net values. For all other metering points, the standard in and out quantities are taken into account. In the report, both net quantities and standard quantities are aggregated in the netInQuantity and netOutQuantity columns. (netInQuantity).
  - Net amounts leaving the grid, kWh. For bidirectional grid metering points with net quantities (netIn and netOut) submitted to the Estfeed Datahub, the report uses these net values. For all other  metering points, the standard in and out quantities are taken into account. In the report, both net quantities and standard quantities are aggregated in the netInQuantity and netOutQuantity columns. (netOutQuantity).
  ...
  - Amounts leaving an isolated grid in the balance responsible party’s portfolio to the end customer, kWh – Pout amount leaving the grid of the grid operator (all isolated metering points that have a valid grid agreement and are not a GO-GO border point) (outQuantityIsolated)
  - Net amounts entering the isolated grid, kWh. For bidirectional isolated grid metering points with net quantities (netIn and netOut) submitted to the Estfeed Datahub, the report uses these net values. For all other isolated metering points, the standard in and out quantities are taken into account. In the report, both net quantities and standard quantities are aggregated in the netInQuantityIsolated and netOutQuantityIsolated columns. (netInQuantityIsolated).
  - Net amounts leaving the isolated grid, kWh. For bidirectional isolated grid metering points with net quantities (netIn and netOut) submitted to the Estfeed Datahub, the report uses these net values. For all other isolated metering points, the standard in and out quantities are taken into account. In the report, both net quantities and standard quantities are aggregated in the netInQuantityIsolated and netOutQuantityIsolated columns. (netOutQuantityIsolated).
  - Number of metering points, pcs (meteringPointsTotal)
- **PAGE "BH_GO_OS_GO"**
  ...
  - Net amounts entering the grid, kWh. For bidirectional grid metering points with net quantities (netIn and netOut) submitted to the Estfeed Datahub, the report uses these net values. For all other metering points, the standard in and out quantities are taken into account. In the report, both net quantities and standard quantities are aggregated in the netInQtyPortfolioLastResortSupply and netOutQtyPortfolioLastResortSupply columns. (netInQtyPortfolioLastResortSupply).
  - Net amounts leaving the grid, kWh. For bidirectional grid metering points with net quantities (netIn and netOut) submitted to the Estfeed Datahub, the report uses these net values. For all other metering points, the standard in and out quantities are taken into account. In the report, both net quantities and standard quantities are aggregated in the netInQtyPortfolioLastResortSupply and netOutQtyPortfolioLastResortSupply columns. (netOutQtyPortfolioLastResortSupply).
...
- Period End – report generation date or, if the grid agreement ended during the current month, the agreement end date (periodEnds)
- Positions - Number of positions by resolution for a unique agreement combination (positions)
- Number of Transmissions of Entered Quantities – total count of transmitted metering data (including zero quantities) (inCount)
- Number of Transmissions of Exited Quantities – total count of transmitted metering data (including zero quantities) (outCount)
- Net Positions – Number of net positions by resolution for a unique agreement combination (netPositions) (electricity market only)
- NetIn Quantity Transmissions Count (In) – total count of transmitted netIn quantities (including zero quantities) (netInCount) (electricity market only)
- NetOut Quantity Transmissions Count (Out) – total count of transmitted netOut quantities (including zero quantities) (netOutCount) (electricity market only)
- Net Quantity Transmissions Count (in = out = 0) – Number of transmitted net positions where both netIn and netOut are equal to zero (netPositionsNCount) (electricity market only).
...
- Net Production (kWh) – net quantity Entered to the grid (netInKwh) (electricity market only)
- Net Consumption (kWh) – net quantity Exited from the grid (netOutKwh) (electricity market only)
