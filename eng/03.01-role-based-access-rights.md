...
| Datahub user type | Market | Abbreviation | Description | Open supply portfolio agreement obligation |
|---|---|---|---|---|
| Grid operator | Electricity, Gas| GO | Own code range. Responsible for transmitting the technical data, information on grid agreements and metering data of the metering points in its grid area. | Yes. Upon the absence of a agreement, Elering will enter it in the open supplier portfolio of the superior grid operator. |
| Line operator | Electricity| LO | Own code range. Responsible for the collection and transmission of information related to its metering points and metering data. | No. Not included in balancing – open supply agreements are not added to line operator metering points. |
| Closed distribution network | Electricity | CDN | Own code range. Responsible for transmitting the technical data, information on grid agreements and metering data of the metering points in its grid area. | Yes. |
| Producer operator | Electricity, Gas| PO | Own code range. Responsible for the collection and transmission of information related to its metering points and metering data. | No, not included in balancing – open supply agreements are not added to PO metering points. |
| Charging point operator | Electricity | CO | Own code range. Responsible for the collection and transmission of information related to its metering points and metering data. | No, not included in balancing – open supply agreements are not added to CO metering points. |
| Open supplier | Electricity, Gas| OS | Open suppliers transmit the information on open supply agreements entered into with market participants and also the portfolio agreement if they provide open supply services to another open electricity undertaking. | Yes. In the absence of an open supply agreement, the agreements entered into by an open supplier with a market participant are rendered null and void.|
| Aggregator | Electricity | AGR | Own code range. Aggregators enter the information on aggregation agreements entered into with market participants and transmit the data on consumption management. | Yes. In the absence of an open supply agreement, the agreements entered into by an aggregator with a market participant are rendered null and void.|
| Energy service provider | Electricity, Gas | ES | Energy service providers receive access to information only on the basis of the access rights granted by end-users. | No – the role has access to data only on the basis of access rights. |
| Gas Station Operator | Gas | GST | Own code range. Responsible for the collection and transmission of information related to its metering points and metering data. | No, not included in balancing – open supply agreements are not added to GST metering points. |
...
| Information Provided | Network Operator (GO) | Closed Distribution Network (CDN) | Line Owner and Producer (LO and PO) | Charging point (CO) | Gas Station Operator (GST) | Data Hub Role | Explanation |
|----------------------|----------------------|-----------------------------------|-------------------------------------|--------------------|----------------------------------|---------------|-------------|
| Customer data | YES | YES | YES | YES | YES | Encodes the customer using EIC (X) code. | |
| Metering point technical data encoded within own code range, and data on the registered network contract party and start/end dates | YES | YES | YES | YES | YES | Format validation, storage, data exchange. | Technical data format. Code range must be obtained from the Data Hub in advance. |
| Hourly metering data per metering point (later 15‑minute metering data) | YES | YES | YES | YES | YES | Format validation, storage, data exchange. | Obligation based on network rules |
| Response to open supply contract change requests | YES | YES | NO | NO | NO | | If no nominated supplier contract exists |
| Network operator’s nominated supplier contract | YES | YES | NO | NO | NO | Storage of contract information and ensuring access rights | The nominated supplier provides open supply on behalf of the network operator to universal service customers |
| Joint invoice contract with open supplier | YES | YES | NO | NO | NO | Storage of contract information and ensuring data exchange | The open supplier issues the network invoice to the customer instead of the network operator |
| Network invoice joint billing to customer | YES | YES | NO | NO | NO | Data exchange | _same_ |
| Response to customer inquiries to the open supplier | YES | YES | NO | NO | NO | _same_ | _same_ |
| Confirmation of disconnection request related to joint billing to the open supplier | YES | YES | NO | NO | NO | _same_ | _same_ |
| Network invoice issuance message | YES | YES | NO | NO | NO | _same_ | _same_ |
| Information exchange to other users | YES | YES | YES | YES | YES | _same_ | Information exchange to other users – all or selected open suppliers, etc. |
...
| Access to information | Grid operator | Closed distribution network | Line operator and producer | Charging point | Gas Station Operator| Description |
|---|---|---|---|---|---|---|
|Customer data|YES|YES|YES|YES|YES||
| Own code range of metering points | YES | YES | YES | YES | YES | For entering metering point information|
| Metering point technical data, grid agreement person, start time and end time | YES | YES | YES | YES | YES | Access to data of its own area |
| Data on agreements related to metering points (persons, start time and end time of open supply and aggregation agreements) | YES | YES | NO | NO | NO | Access to data of its own area |
| Metering data of metering points | YES | YES | YES | YES | YES | Access to data of its own area |
| Metering data of metering points, including border metering points | YES | YES | YES | YES | YES | Access to data: grid operator as a customer |
| Metering data transmission status | YES | YES | YES | YES | YES | New. The Datahub monitors data exchange for every metering point. Data of own metering points. |
| Data on the consumption management by the aggregator of metering points | YES | YES | YES | YES | YES | New. Basis: Network Code on the Operation of the Electricity Market (currently draft). Regarding own metering points. |
| Data on requests for amendment of open supply agreements | YES | YES | NO | NO | NO | Access to data of its own area |
| Registered agreements in own area: joint invoice agreement, named supplier agreement, open supplier and balance responsible party of the area | YES | YES | NO | NO | NO | Access to data of its own area |
| Disconnection application and customer enquiry messages in relation to joint invoice from open supplier and own answers thereto | YES | YES | NO | NO | NO | Access to data of its own area |
| Messages related to information exchange | YES | YES | YES | YES | YES | Information exchange between users |
| Calculation of grid losses | YES | YES | NO | NO | NO| New functionality. |
| Summary reports by agreements related to own area | YES | YES | YES | YES | YES| Content of reports described separately. |
