...
Each grid operator, open supplier and aggregator must have an portfolio agreement with another open supplier for its portfolio, called a portfolio agreement in the Datahub (while the consumer and the producer
have open supply agreements for metering points, the grid operator, aggregator and open supplier have open supply agreements per legal entity).
...
If an aggregator does not have a portfolio agreement, it cannot act as an aggregator and the system operator terminates its operations and aggregation agreements.
...
| Rule                                                                                                                             | Electricity market | Gas market |
|------------------------------------------------------------------------------------------------------------------------------------|--------------|-----------|
| The start date of the agreement can only be the first day of the month.                                                                               | YES          | YES       |
| The start date of the agreement must be at least seven days in the future.                                                                             | 7 days      | 14 days  |
| The end date of the agreement can only be the last day of the month.                                                                                | YES          | YES       |
| The end date of the agreement must be at least seven days in the future.                                                                               | 7 days      | 14 days  |
| There must be no metering points in the agreement dataset.                                                                                   | YES          | YES       |
| The agreement must not stipulate an early termination fee.                                                                              | YES          | YES       |
| The agreement cannot be deleted if there are less than seven days left until the agreement expires.                                                | 7 days | 14 days |
| Only a market participant in the role of an open supplier (OS) can add the agreement. | YES | YES |
| The `serviceProvider` of the agreement must be the registrant of the agreement and the `customer` of the agreement may be any market participant.    | YES          | YES       |
| Adding an agreement to oneself is not allowed (`serviceProvider` and `customer` must not be the same). | YES          | YES       |
