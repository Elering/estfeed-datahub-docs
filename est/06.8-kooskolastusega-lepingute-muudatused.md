...
* [Kooskõlastust nõudvad lepingumuudatused](#kooskõlastust-nõudvad-lepingumuudatused)
  ...
  * [Lepingute kooskõlastused veebiliideses](#lepingute-kooskõlastused-veebiliideses)
    ...
    * [Olemasoleva kooskõlastust vajava lepingu muutmine või kustutamine](#olemasoleva-kooskõlastust-vajava-lepingu-muutmine-või-kustutamine)
...
1. Avatud tarnija sisestab taotluse:
   2. muuta avatud tarne lepingu algus- või lõppkuupäeva VÕI
   3. lisada uue avatud tarne lepingu VÕI
   4. kustutada olemasolev avatud tarne leping
...
### Olemasoleva kooskõlastust vajava lepingu muutmine või kustutamine

1. Ava avatud tarnija rollis leht "Lepingud".
2. Leia leping, mida soovid muuta või kustutada.
3. Vajuta nuppu "Kooskõlastus".
![Kooskõlastus nupp](../images/opp-ui/agreement-coordination/agreement-coordination-coordination-button-est.png)
4. Süsteem avab modaalakna. Kõigepealt vali soovitud toiming: Muuda või kustuta leping
![Muutmise või kustutamise modal](../images/opp-ui/agreement-coordination/agreement-coordination-modify-delete-modal-est.png)
5. Sõltuvalt tehtud valikust:
* Muuda - vajadusel uuenda lepingu kuupäevi ja lisa muudatuse põhjendus.
![Muutmise modal](../images/opp-ui/agreement-coordination/agreement-coordination-modify-delete-modal-modify-est.png)
* Kustuta - lisa muudatuse põhjendus.
![Kustutamise modal](../images/opp-ui/agreement-coordination/agreement-coordination-modify-delete-modal-delete-est.png)
6. Vajuta nuppu "Algata", et edastada leping kooskõlastusse.
7. Kooskõlastus on algatatud ja leitav lehelt "Lepingute kooskõlastamine".
...
![Otsi nupp](../images/opp-ui/agreement-coordination/agreement-coordination-search-button-est.png)
3. Tulemustes kuvatakse lepingute kooskõlastused koos info ja kinnitajatega.
![Otsingu tulemused](../images/opp-ui/agreement-coordination/agreement-coordination-search-result-est.png)
4. Vajutades kinnitajate tabelis dokumendi ikoonile on võimalik vaadata ka olemasolevaid seotud lepinguid.
![Seotud lepingud nupp](../images/opp-ui/agreement-coordination/agreement-coordination-related-agreement-est.png)
...
| Sõnum                                                      | Eesmärk                                                                            |
|------------------------------------------------------------|------------------------------------------------------------------------------------|
| `POST /api/{version}/agreement-coordinations/bulk`         | Lepingu lisamise/muutmise/kustutamise taotluste loomine                            |
| `GET /api/{version}/agreement-coordinations/{id}`          | Lepingu lisamise/muutmise/kustutamise taotluse otsimine ID alusel                  |
| `GET /api/{version}/agreement-coordinations`               | Lepingu lisamise/muutmise/kustutamise taotluste otsimine erinevate tunnuste alusel |
| `POST /api/{version}/agreement-coordinations/{id}/cancel`  | Lepingu lisamise/muutmise/kustutamise taotluse tühistamine                         |
| `POST /api/{version}/agreement-coordinations/{id}/approve` | Lepingu lisamise/muutmise/kustutamise taotluse kooskõlastamine                     |
| `POST /api/{version}/agreement-coordinations/{id}/deny`    | Lepingu lisamise/muutmise/kustutamise taotluse kooskõlastusest keeldumine          |
...
- Olemasoleva avatud tarne lepingu muutmisel tuleb esitada soovitud kuupäevad:
    - `proposedValidFrom` näitab uut soovitud lepingu alguskuupäeva (peab olema mitte hilisem kui 31.12.2035);
    - `proposedValidTo` näitab uut soovitud lepingu lõppkuupäeva (peab olema kas NULL või varasem kui 01.01.2080).
- Uue avatud tarne lepingu lisamisel tuleb esitada lisaks tavapärased avatud tarne lepingu andmed.
- Olemasoleva avatud tarne lepingu kustutamisel tuleb esitada lepingu ID
...
- `agreementAction` näitab, kas tegu on lepingu loomisega, uuendamisega või kustutamisega. Võimalikud valikud on: `CREATE`, `UPDATE` ja `DELETE`.
